    ...
    def encode(self, text):
        ...
        for token in re.findall(self.pat, text):
            ...
            for bpe_token in bpe_result:
                # ✅ 关键修复：防止 KeyError + 防止越界
                if bpe_token in self.encoder:
                    idx = self.encoder[bpe_token]
                else:
                    # ⚠️ 用已有 token（不能新增 vocab）
                    idx = self.encoder['<|endoftext|>']

                bpe_tokens.append(idx)

        # ✅ 最后再做一层保险（防 CUDA 崩）
        max_id = len(self.encoder) - 1
        bpe_tokens = [min(max(t, 0), max_id) for t in bpe_tokens]
